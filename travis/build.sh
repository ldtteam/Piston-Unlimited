...
if [ "$TRAVIS_PULL_REQUEST" == "false" ] && [ -n "${GITHUB_TOKEN:-}" ]; then
      curl -H "Content-Type: application/json" -H "Authorization: token $GITHUB_TOKEN" \
            --request POST \
            --data '{"state":"pending",
            "target_url":"https://travis-ci.org/multipiston/multipiston/builds/'$TRAVIS_BUILD_ID'",
            "description": "Build in progress...",
            "context":"build"}' \
            "https://api.github.com/repos/multipiston/multipiston/statuses/${TRAVIS_COMMIT}"
