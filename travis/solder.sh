...
if [ "$TRAVIS_PULL_REQUEST" == "false" ] && ( any "beginswith $TRAVIS_BRANCH" develop master release version ); then
    ...
    zip -r multipiston.zip mods
    echo $(unzip -l multipiston.zip)
    
    JAR_VERSION=$(echo $JAR_FILE | cut -d'-' -f4 | rev | cut -c5- | rev)
    echo $JAR_VERSION
    
    ZIP_NAME=multipiston-$JAR_VERSION.zip
    
    mv multipiston.zip $ZIP_NAME
    
    curl -T $ZIP_NAME -u $SOLDER_USER:$SOLDER_PASS $SOLDER_FTP/multipiston/
    ...
    MODVERSION_ID=$(curl -s -b cookies.txt -c cookies.txt $SOLDER_URL/modpack/build/$MODPACK_VERSION | awk '/(multipiston)/,/Forge/' | grep modversion_id | head -n 1 | cut -d'"' -f8)
